...
## v1.4.0 · 一副声音：原生语音条与受控语音工具

文字与文件之后是声音。[`code/voice_sender.py`](code/voice_sender.py) 把「一句话文本 → TTS → AMR-NB → 语音条」封装成受控工具：应用门走 `media/upload?type=voice` → `message/send(msgtype=voice)`，客服门走 `kf/send_msg(msgtype=voice)` 并占用与文本相同的五格回复预算。防护五层各有消融项（见 [`tests/ABLATION.md`](tests/ABLATION.md)）：文本长度上限在 TTS 之前拦截、按解码后 PCM 实际时长校验 60 秒边界、编码后按 2MB API 上限校验字节、应用收件人白名单、客服绑定客户校验。
...
## v1.6.0 · 共听：Agent 在聊天里发歌卡，你点开歌就在放

把一台完整的网页音乐播放器挂进底部菜单：六个页签（歌单/搜索/云歌单/FM/日推/播客）、全屏播放器（队列、留言、±15 秒、译文/罗马音歌词）、断点续播。Agent 在聊天里丢一张歌卡，点开落地即播；用户自己的队列不清场——客歌插队，放完自动接回原来那首、原来那一秒。页面把播放状态上报后端，bot 每轮按新鲜度把「正在听什么」注入模型上下文，Agent 接话就像坐在旁边。

```text
个人微信 ─ 底部菜单 / 歌卡 ─> /music 单文件页面 ─> /api/music/* 后端代理 ─> 网易云音乐 API
                                   └ POST now-playing ─> bot 注入「正在听什么」
```

前端参考实现：[`code/co_listen_page.html`](code/co_listen_page.html)（零构建零依赖单文件，微信 WebView 七个坑的解法都在里面）。后端只发接口契约，音源接法（扫码登录、登录态保存与过期续命）、歌卡「链接必须干净」的安检教训、歌词三轨同步原理、共听注入的三条纪律，见专题教程：[guide/共听.md](guide/共听.md)
...
| 语音条发不出去或收到不能播 | `type=voice` 素材只认 AMR-NB（8kHz 单声道，文件头 `#!AMR\n`），且 ≤2MB、≤60秒；MP3/WAV 直接当voice上传会被拒。转码与逐帧编码的完整套路见 [原生语音条](guide/原生语音.md)。 若手机能放、**电脑微信**（含微信里的企微插件）点开是嘟嘟电流，那是桌面微信不解接口上传的语音，两扇门都一样、换格式无效，电脑上用电脑版企业微信听；详见 [原生语音](guide/原生语音.md)。 |
